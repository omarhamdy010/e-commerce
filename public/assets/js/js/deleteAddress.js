...
httprequest.open("GET","http://aaaserver-001-site31.ftempurl.com/"+lang+"/api/Address/GetAddresses?Index=0&Size=10");
...
httprequest.onreadystatechange=function(){
   ...
   if(Data.length == 0){
document.getElementById("deleteAddress").innerHTML='<p>There is no address</p>'
   }
...
function diplayAddress(Data)
{
    address = "";
          for(var i =0 ;i<Data.length ; i++)
              {
                  ...
                  <span >`+Data[i].street+" "+Data[i].floor+`</span>
                  </div>
                  <i  id="`+Data[i].id+`" onclick="deleteaddress(this.id)" style="float:right; color:red; font-size:20px" class="fa fa-trash"></i>
                  <div class="clear"></div>
                  </div>
                  `
          ...
          document.getElementById("deleteAddress").innerHTML = address;
              if(Data.length==0){
                document.getElementById("deleteAddress").innerHTML='<p>There is no address</p>'
              }
}
function deleteaddress(event){
    ...
    var token=JSON.parse(sessionStorage.getItem('token'));
    fetch("http://aaaserver-001-site31.ftempurl.com/"+lang+"/api/Address/DeleteAddress",{
        method:'DELETE',
        body:JSON.stringify({
            address_Id:id
        }),
